...
//
// SERVERLESS (Vercel): cada ruta API tiene su propio proceso aislado.
// globalThis no se comparte entre rutas, solo entre invocaciones CÁLIDAS
// de la MISMA ruta. Por eso max:1 — una ruta serverless procesa una request
// a la vez; más de 1 conexión por instancia solo acumula conexiones idle.
//
// PgBouncer Transaction mode (puerto 6543): libera la conexión de servidor
// apenas termina cada transacción, por lo que max:1 es correcto y seguro.
// keepAlive:false evita que el pool fuerce conexiones a permanecer abiertas
// entre invocaciones, que es la causa principal del agotamiento de conexiones.
function createPool() {
  return new Pool({
    host:     process.env.DB_HOST,
    port:     parseInt(process.env.DB_PORT || '6543'),  // 6543 = PgBouncer Transaction (NO Session 5432)
    database: process.env.DB_NAME,
    user:     process.env.DB_USER,
    password: process.env.DB_PASSWORD,
    ssl:      process.env.DB_SSL === 'false' ? false : { rejectUnauthorized: false },
    max:                     1,     // Serverless: 1 conexión por instancia es suficiente y previene EMAXCONN
    idleTimeoutMillis:       10000, // Libera conexiones idle en 10s (antes 30s acumulaba conexiones en instancias warm)
    connectionTimeoutMillis: 5000,
    allowExitOnIdle:         true,  // Permite que el pool se cierre cuando no hay queries — libera recursos en serverless
    // keepAlive omitido (false por defecto): no mantener conexiones TCP vivas innecesariamente en serverless
  })
}
