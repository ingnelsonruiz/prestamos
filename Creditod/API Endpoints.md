...
| GET | `/api/dashboard/intereses-recogidos-detalle` | Desglose de intereses ya cobrados por crédito. Retorna `{ normales[], libres[], totales }` (mismo formato que `intereses-detalle`, corregido 2026-08-05 y 2026-08-16 — ver abajo) |
...
>
> ⚠️➡️✅ **Bug corregido (2026-08-16) — `normales` prorrateaba desde `cu.monto_cuota`/`abono_interes` (mutables), no desde el pago real.** Hasta esa fecha, tanto esta query `normales` como su drill-down (`.../pagos/route.js`, rama `tipo='normal'`) y la query 2 de `GET /api/dashboard` calculaban `LEAST(pg.monto, cu.monto_cuota) * cu.abono_interes / NULLIF(cu.monto_cuota, 0)`. El problema: en método `plano`, `recalcularCuotasPlano()` reescribe `cu.monto_cuota`/`abono_interes` después de cada pago del crédito — cuando una cuota se cierra "solo intereses", el capital pendiente se redistribuye entre menos cuotas restantes, **inflando** el `monto_cuota` de la cuota aún abierta. Dividir entre ese valor inflado prorrateaba un pago 100% interés a una fracción mínima. Caso real: CRED-000309, pago de $1.000.000 (100% interés) mostrado como $130.435. **Fix**: se reemplazó por `pg.monto_interes` directo (mismo patrón ya usado, correctamente, para `credito_libre`) en los tres lugares. Detalle completo, con la reconstrucción numérica del caso, en [[Incidentes y Bugs Conocidos]].
>
> **Patrón a vigilar (actualizado 2026-08-16)**: el interés cobrado de un pago **nunca** debe derivarse de `cred_cuotas.abono_interes`/`monto_cuota` — ambos son mutables y `recalcularCuotasPlano()` los reescribe retroactivamente tras cada pago posterior del mismo crédito plano. Para `tipo='credito_libre'` además `abono_interes` es un placeholder fijo en `0` (ver `Créditos Sin Cuotas Futuras.md`). En ambos casos el valor correcto es **siempre** `cred_pagos.monto_interes`, calculado e inmutable al momento del cobro. Cualquier vista que agregue "intereses cobrados" a nivel de negocio debe sumar **las tres fuentes** (`cred_pagos.monto_interes` de préstamos normales, `cred_pagos.monto_interes` de créditos libres, `cred_retornos_empresa.monto_interes`) — omitir cualquiera, o volver a prorratear desde la cuota, reproduce este mismo bug en otro reporte.
...
Reutiliza exactamente la misma fórmula que `intereses-recogidos-detalle/route.js` (`pg.monto_interes` directo para `normal` y `libre` — unificado así desde el fix 2026-08-16, antes `normal` prorrateaba con `LEAST(pg.monto, cu.monto_cuota) * cu.abono_interes / NULLIF(cu.monto_cuota,0)`, ver callout arriba; `r.monto_interes` para `retorno`) — el `total_interes` de esta respuesta **siempre debe coincidir** con `interes_cobrado` de la fila que lo originó en el modal padre. Para `tipo='normal'` el `SELECT` sigue trayendo `cu.numero_cuota`/`monto_cuota`/`abono_interes_cuota`/`estado` como información de contexto para la UI (qué cuota era, su estado), pero esos campos ya **no** participan del cálculo de `interes_prorrateado`. Si un pago de tipo `retorno` no tiene crédito asociado (`producto_id` null en la fila agregada), la query filtra explícitamente `r.producto_id IS NULL` en vez de intentar un match por igualdad que nunca encontraría nada.
