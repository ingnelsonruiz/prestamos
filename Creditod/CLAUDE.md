...
13. **El interés cobrado de un pago (método `plano`) NUNCA se debe re-derivar de `cred_cuotas.monto_cuota`/`abono_interes`** — ambos son mutables y `recalcularCuotasPlano()` los reescribe (a veces muy inflados) después de cada pago posterior del mismo crédito. El valor correcto y siempre disponible es `cred_pagos.monto_interes` (y `monto_capital`), calculado e inmutable en el momento exacto del cobro. Corregido en `GET /api/dashboard` (query 2) e `intereses-recogidos-detalle` (+ su drill-down `.../pagos`) el 2026-08-16 tras detectar un pago 100% interés de $1.000.000 mostrado como $130.435 — ver [[Incidentes y Bugs Conocidos]].
