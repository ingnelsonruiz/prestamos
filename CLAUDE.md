...
**Estado: corregido y confirmado por Nelson (2026-08-19).** El archivo se sincronizó a `C:\programa alberto mario\Programa_Creditos\app\api\migracion\reset-cliente\route.js` y el usuario confirmó que ya funciona ("listo") tras el despliegue — "Limpiar cliente específico" ya borra correctamente créditos que participaron en una unificación, limpiando primero su(s) fila(s) en `cred_unificaciones`.

---

## 28. Fix — `cred_pagos.monto_interes`/`monto_capital` nunca se llenaban en liquidaciones anticipadas — 2026-08-21

### Síntoma

En el dashboard, el modal **"Detalle de intereses recogidos"** (`GET /api/dashboard/intereses-recogidos-detalle` — endpoint agregado el 2026-08-16, no documentado hasta esta sesión en este archivo) no mostraba un crédito **Sin Cuotas Futuras** (`CRED-000683`, cliente Carlos Hernán Gutierres Yeni Peñaloz) que acababa de liquidarse anticipadamente con $500.000 de interés cobrado (recibo `REC-000817`) — pese a que la ficha del crédito (`/creditos-libres/[id]`), la tabla de cuotas y el historial de pagos sí mostraban ese interés correctamente. El total combinado del widget quedaba subestimado exactamente en el interés de esa liquidación ($1.767.312 mostrado vs. $2.267.312 real ese día).

### Causa raíz

`POST /api/productos/[id]/liquidar` (usado tanto por "Liquidar crédito" como por "🏁 Recoger crédito" desde `/cobros`, para **cualquier** tipo de crédito, incluidos `credito_libre`) calcula correctamente el desglose capital/interés de la cuota consolidada (`capitalCuotaRef`, `interesCuotaRef`) y lo persiste en `cred_cuotas.abono_capital`/`abono_interes` — pero el `INSERT INTO cred_pagos` del paso 4 **nunca incluía las columnas `monto_interes`/`monto_capital`**, a diferencia de `POST /api/pagos` (flujo de cuotas normales) y de `POST /api/creditos-libres/[id]/abonar`, que sí las llenan siempre (§8, §18).

El widget `intereses-recogidos-detalle` fue diseñado para leer el interés **directamente de `cred_pagos.monto_interes`** en vez de derivarlo de `cred_cuotas` — precisamente para evitar el bug ya documentado de `recalcularCuotasPlano()` mutando `abono_interes` después de pagos posteriores (caso `CRED-000309`, ver comentario de cabecera del propio endpoint). Para créditos libres exige además `pg.monto_interes > 0` en el `WHERE`; para créditos normales exige `HAVING SUM(pg.monto_interes) > 0` por crédito. La columna `cred_pagos.monto_interes`/`monto_capital` tiene `DEFAULT 0` (verificado en producción, Supabase `HERMANOS_LIÑAN`) — al omitirla en el `INSERT`, Postgres la deja en `0`, **no** en `NULL`. Como `0 > 0` es falso igual que `NULL > 0`, el efecto práctico es el mismo: el pago
- Desaparece por completo de la sección "Créditos Sin Cuotas" cuando fue el único pago del crédito (caso `CRED-000683`).
- O, si el crédito ya tenía pagos normales previos con `monto_interes` correcto, subestima el interés total de ese crédito en el monto de la liquidación (sección "Créditos normales").

El bug **no es exclusivo de créditos libres**: afecta cualquier liquidación anticipada de cualquier tipo de crédito, porque el `INSERT` afectado es genérico a `liquidar/route.js`, compartido por todos los tipos.

### Fix

`app/api/productos/[id]/liquidar/route.js`, paso 4: el `INSERT INTO cred_pagos` ahora incluye `monto_interes` y `monto_capital`, usando los mismos valores (`interesCuotaRef`, `capitalCuotaRef`) que el paso 1 ya calculaba y persistía en `cred_cuotas` — no se recalculó nada nuevo, solo se replicó el desglose ya existente también en el pago.

### Backfill de liquidaciones anteriores al fix

`27_backfill_intereses_liquidacion.sql`: corrige retroactivamente los pagos de "LIQUIDACIÓN ANTICIPADA" ya registrados con `monto_interes = 0` y `monto_capital = 0` (criterio corregido el mismo día tras verificar en producción que la columna usa `DEFAULT 0`, no `NULL` — la primera versión del script buscaba `IS NULL` y no encontraba nada), exigiendo además que la cuota asociada tenga `abono_capital > 0` (toda liquidación real mueve capital, así que un `monto_capital=0` junto a una cuota con capital consolidado es la señal inequívoca del bug, y evita tocar algún caso legítimo). Corrige **solo el caso no ambiguo** — cuotas que recibieron un único pago en toda su vida (la propia liquidación), donde `cred_cuotas.abono_interes`/`abono_capital` de esa cuota son exactamente el desglose de ese pago. Si una cuota tuvo pagos parciales previos a la liquidación, el script no la toca (no se puede reconstruir el desglose exacto por SQL puro sin conocer el detalle histórico de esos pagos previos) y la deja listada en un `SELECT` final de reporte para revisión manual.
...
**Estado: corregido, desplegado y confirmado por Nelson (2026-08-21).** El backfill (`28_fix_credito_libre_mal_creado.sql`) se corrió en Supabase (proyecto `HERMANOS_LIÑAN`): el diagnóstico encontró **7 créditos afectados** — `CRED-000476`, `CRED-000612`, `CRED-000646`, `CRED-000689`, `CRED-000716`, `CRED-000723`, `CRED-000739` —, todos en el caso seguro (sin ningún pago registrado, sin ningún caso ambiguo). El `UPDATE` los corrigió a los 7; verificado con el reporte final del paso 3, que devolvió 0 filas. El código (`app/migracion/cargue-inicial/page.js`, `app/api/migracion/cargue-inicial/route.js`, `app/prestamos/nuevo/page.js`) ya está desplegado (`git push` confirmado) — los formularios ya no ofrecen `credito_libre` como tipo seleccionable y el endpoint de Cargue Inicial lo rechaza con 400 si llegara de todas formas.

**Nota:** `CRED-000476` (el más antiguo de los 7, capital $20.000.000) tenía `fecha_vencimiento = 2023-09-15` — muy anterior a su `fecha_creacion` (2026-07-13) — consistente con que Cargue Inicial está diseñado para legalizar deudas antiguas con fecha de desembolso en el pasado. Si en el futuro aparece un `credito_libre` nuevo con este mismo síntoma (en mora casi de inmediato tras el desembolso), es señal de que existe una **tercera** vía de creación de créditos aún no identificada que tampoco filtra `comportamiento='sin_cuotas_futuras'` — revisar `POST /api/productos/unificar` (§21) y cualquier script de migración/importación nuevo que use `generarCuotas()`.
